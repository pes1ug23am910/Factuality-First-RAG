...
def main() -> None:
    ...
    if args.mock:
        ...
        references = ["Paris", "Shakespeare", "Deoxyribonucleic acid"]
    else:
        from datasets import load_dataset  # type: ignore[import-untyped]
        ds = load_dataset(args.dataset, split=args.split)
        indices = np.random.choice(len(ds), size=min(args.sample, len(ds)), replace=False)
        queries = [ds[int(i)]["question"] for i in indices]
        references = [ds[int(i)]["answer"]["value"] if isinstance(ds[int(i)]["answer"], dict) else str(ds[int(i)]["answer"]) for i in indices]
    ...
    for i, (q, ref) in enumerate(zip(queries, references)):
        ...
        gate_retrieve = pipe_full.probe.should_retrieve(q)
